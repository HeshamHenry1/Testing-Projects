...
test.describe('check product',async()=>{
    let Login : LoginPage 
    let Product : productPage
    test('check product detailes  ', async ({ page }) => {
       ...
       await test.step('5- check products  ',async()=>{
        await product.checkProductVisiable();
       })
       await test.step('5- check all products have images name price   ',async()=>{
        await product.verifyAllProductsHaveNameImagePrice();
       })
